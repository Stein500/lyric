# 🎬 PROGRESS — « L'amour est la réponse » — Daïsky

**Style :** Mixte — amour/légèreté/chaleur + touches cyan/ambre (écart à la charte Dark Trap validé par l'artiste).
**Format demandé :** TikTok 9:16 (1080×1920).
**Audio :** `L'amour est la réponse - Daïsky.mp3` — durée exacte **214,92 s (3:34,9)**.

---

## ✅ État d'avancement

- [x] **Étape 1 — Cadre** : dossiers, `.gitignore` (livrables/ PAS exclu), `PROGRESS.md`.
- [x] **Étape 2 — Image d'ancrage** : validée.
- [x] **Étape 3 — Salve 1** : images 01–10.
- [x] **Étape 4 — Salve 2** : images 11–20.
- [x] **Étape 5 — Salve 3** : images 21–30.
- [x] **Étape 6 — 30 images** : un vers = une image HD 9:16 (tous les vers couverts).
- [x] **Étape 7 — Vidéo 9:16 v1** (salve 1 seule) : livrée.
- [x] **Étape 8 — Vidéo 9:16 v2** (30 images + effet vague) : livrée.
  - Apparition/disparition de chaque vers avec **EFFET VAGUE** (lettres qui montent/descendent en ondulation + fondu décalé lettre par lettre).
  - Intro **musicale** (aucune parole) ; outro **musicale** (couple au lever de soleil, sans paroles).
- [ ] **Étape 9 — Commit + push de la v2** : à faire.

---

## 🖼 Images (30 portraits 9:16 — 1 vers = 1 image)

### Salve 1 (01–10)
01 Refrain « qui tu es » · 02 Intro musicale · 03 Danse/sans frontières · 04 Couplet1 chaînes · 05 Pré-refrain cœurs · 06 Refrain2 unité · 07 Couplet2 flamme · 08 Sans frontières · 09 Homme/femme/enfant · 10 Final union

### Salve 2 (11–20)
11 D'où tu viens · 12 Noir/blanc/riche/pauvre · 13 Une seule race · 14 Peau = écrin/cœur trésor · 15 Sans genre/âge/corps · 16 Laisse ton âme se libérer · 17 Rock évangile · 18 Viens on chante · 19 Même humanité · 20 Danser la liberté

### Salve 3 (21–30)
21 Amour sans nom/loi · 22 Cœurs qui s'embrasent · 23 Âmes se mélanger/réparer · 24 Laisse-la brûler · 25 Monde géant/battements · 26 Cri de ralliement Wolof · 27 Refrain variante · 28 Refrain pareils · 29 Outro lever de soleil · 30 Final ensemble puissants

---

## 🎬 Livrables TikTok 9:16

- **v1** : `livrables/L'amour_est_la_reponse_Daïsky_9x16_v1.mp4` — 14 MB (salve 1).
- **v2** : `livrables/L'amour_est_la_reponse_Daïsky_9x16_v2.mp4` — **20 MB**, 1080×1920, 24 fps, 3:34,90, **effet vague** sur les vers, badge statique, **0 trou noir**.

---

## ⚠️ Notes

1. **Intro musicale** : « Wolof TechStein beat wê ! » = pur musical, aucune parole incrustée (image + musique).
2. **Outro musicale** : paroles jusqu'à 2:56, audio 3:34,9 → outro ~39 s (couple au lever de soleil + badge).
3. **16:9 (YouTube)** : pas encore demandé / fait (priorité 9:16).

---

## 🧰 Env / Outils

- venv `.venv` (pillow, mutagen, imageio-ffmpeg) — NON commité.
- **FFmpeg** fourni par `imageio-ffmpeg` (binaire statique, hors dépôt).
- **Polices** : GreatVibes (cursive) + Pacifico (badge) dans `assets/fonts/`.
- `.venv/` et `work/` sont régénérés localement (exclus des snapshots).

---

**Signature :** « Wolof TechStein beat wê ! » ⚡

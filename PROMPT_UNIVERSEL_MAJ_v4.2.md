...
- 30 portraits 9:16 (1080×1920) : la base pour TikTok/Reels/Shorts.
- Les paysages 16:9 natifs sont un bonus ; si indisponibles, utiliser les portraits en mode **letterbox-contain** (bandes noires `#05060A` à gauche/droite), JAMAIS de crop destructif.
...
- Notation `-MM:SS` dans les paroles de l'artiste = **fin du vers** (pas début). Le début est la fin du vers précédent, SAUF pour les vers après 1:50 qui doivent être écrits en plages explicites `m:ss-m:ss`.
...
3. **Frame-by-frame des 30 DERNIÈRES SECONDES** (règle v4.2) : extraire une frame toutes les 2–3 s dans la fin de vidéo (de `DURATION-30` à la fin) + 1 frame de contrôle par section clé (début V1, milieu V2, hook final "never lies", outro Wolof, tout dernier frame du fade). Vérifier visuellement :
   ...
   - le vers "lightning never lies" (ou équivalent) est bien sur son image dédiée
...
- [ ] Paysages 16:9 natifs si possible, sinon fallback letterbox
